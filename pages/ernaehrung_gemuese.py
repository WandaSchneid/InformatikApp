...
input0 = st.number_input("🥕 Karotten (g)", min_value=0, step=5)
kcal_input0 = input0 * 0.4
input1 = st.number_input("🥬 Spinat (g)", min_value=0, step=5)
kcal_input1 = input1 * 0.2

if kcal_input0 + kcal_input1 > 0:
    st.info(f"📊 Gesamt: **{kcal_input0 + kcal_input1:.1f} kcal** (🥕 Karotten: {kcal_input0:.1f} kcal + 🥬 Spinat: {kcal_input1:.1f} kcal)")
...
# Kategorie-ID(s) für Gemüse
category_ids = df_category[df_category["description"].str.contains(r"Vegetables and Vegetable Products", case=False, regex=True)]["id"].unique()
